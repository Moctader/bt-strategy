class TradeManager:
    ...
    def exit(self, transactions, share_price):
        if self.position > 0:
            self.sell(transactions, share_price)
        elif self.in_short_position:
            self.cover_short(transactions, share_price)
        self.add_transaction(transactions, 'exit', 0, share_price)

    # Adjust the current position
    def adjust(self, transactions, share_price):
        # Implement your adjustment logic here
        self.add_transaction(transactions, 'adjust', self.position, share_price)